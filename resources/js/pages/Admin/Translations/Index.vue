...
const draftValue = (row: TranslationRow, locale: string): string =>
    ensureDraft(row)[locale] ?? '';
...
const applyFilters = () =>
    router.get('/admin/translations', filters, {
        preserveState: true,
        replace: true,
    });
const save = (row: TranslationRow) => {
    saving[row.id] = true;
    useForm({ translations: ensureDraft(row) }).put(
        `/admin/translations/${row.id}`,
        {
            preserveScroll: true,
            onFinish: () => {
                saving[row.id] = false;
...
<template>
    <AdminLayout title="Translations">
        ...
        >
            <div>
                ...
                <p class="mt-2 text-slate-500">
                    Edit public website copy. Keys stay fixed so code references
                    remain safe.
                </p>
            </div>
            <div
                v-if="success"
                class="rounded-lg bg-emerald-100 px-4 py-2 text-sm text-emerald-800"
            >
                {{ success }}
            </div>
        </div>
        ...
        <form
            class="mt-8 grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-[1fr_220px_auto]"
            @submit.prevent="applyFilters"
        >
            <Input v-model="filters.search" placeholder="Search key or text…" />
            <select
                v-model="filters.group"
                class="rounded-md border bg-white px-3 text-sm"
            >
            ...
            >
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <Button
                        ...
                        @click="save(row)"
                        >{{ saving[row.id] ? 'Saving…' : 'Save' }}</Button
                    >
                </div>
                <div class="mt-4 grid gap-4 lg:grid-cols-2">
                    ...
                    >
                        ...
                        >
                        <textarea
                            :value="draftValue(row, locale)"
                            rows="3"
                            class="w-full rounded-lg border px-3 py-2 text-sm focus:border-slate-500 focus:outline-none"
                            @input="updateDraft(row, locale, $event)"
                        />
                    </label>
                </div>
            </article>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
            <a
                ...
                v-html="link.label"
                class="rounded-md border bg-white px-3 py-2 text-sm"
                :class="{
