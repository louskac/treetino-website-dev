...
<template>
    ...
    >
        ...
        >
            <div class="max-w-3xl">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    >
                        ...
                        <span>{{
                            $t('media.patent_tag', 'Patentovaná Technologie')
                        }}</span>
                    </span>
                    <span
                ...
                >
                    {{
                        $t(
                            'media.patent_title',
                            'Patentovaný mechanický systém natáčení větví',
                        )
                    }}
                </h2>
                <p
                ...
                >
                    {{
                        $t(
                            'media.patent_desc',
                            'Treetino chrání své inovace evropským i mezinárodním patentem. Naše technologie umožňuje inteligentní vícesměrné natáčení větví, eliminaci stínění a automatickou ochranu proti krupobití.',
                        )
                    }}
                </p>
            </div>
            ...
            <div
                class="flex shrink-0 flex-col items-stretch gap-3 sm:flex-row sm:items-center"
            >
                <a
                    href="/docs/patent-ep-4664750.pdf"
                    target="_blank"
                    download="Treetino-Patent-EP4664750A1.pdf"
                    class="inline-flex items-center justify-center gap-2 rounded-xl bg-t-blue px-4 py-2.5 text-xs font-medium whitespace-nowrap text-white shadow-sm transition hover:bg-t-blue/90"
                >
                    <Download class="h-3.5 w-3.5" />
                    <span>{{
                        $t('media.patent_download_ep', 'EU Patent (PDF)')
                    }}</span>
                </a>
                <a
                    ...
                    class="inline-flex items-center justify-center gap-2 rounded-xl border border-black/15 bg-white px-4 py-2.5 text-xs font-medium whitespace-nowrap text-black/80 shadow-xs transition hover:border-black/30 hover:bg-black/5"
                >
                    <ExternalLink class="h-3.5 w-3.5 text-t-blue" />
                    <span>{{
                        $t('media.patent_download_wo', 'World Patent (PDF)')
                    }}</span>
                </a>
            </div>
        </div>
        ...
        <div
            class="mt-14 grid grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center"
        >
            <!-- Left: Clean 3D Animation (7 Cols) -->
            <div class="lg:col-span-7">
                <div
                    class="relative flex aspect-4/3 w-full items-center justify-center overflow-hidden rounded-3xl border border-black/10 bg-[#fafafa] shadow-xs"
                >
                    <canvas
                        ref="canvasRef"
            ...
            <div class="flex flex-col justify-between space-y-7 lg:col-span-5">
                <div
                    v-for="(feature, idx) in patentFeatures"
                    :key="idx"
                    class="border-l-2 border-t-blue pl-6"
                >
                    <h3 class="text-base font-medium text-black sm:text-lg">
                        {{
                            feature.title[$i18n.locale as 'cs' | 'en'] ||
                            feature.title.cs
                        }}
                    </h3>
                    <p
                        class="mt-1.5 text-xs leading-relaxed text-black/70 sm:text-sm"
                    >
                        {{
                            feature.description[$i18n.locale as 'cs' | 'en'] ||
                            feature.description.cs
                        }}
                    </p>
                </div>
            </div>
