...
new_func = """def draw_page_6(c: canvas.Canvas, data: dict, assets_path: str):
    ...
    c.drawCentredString(285, header_y, "CENA/JEDNOTKU")
    c.drawCentredString(335, header_y, "SLEVA")
    c.drawCentredString(405, header_y, "CELKEM BEZ DPH")
    c.drawCentredString(475, header_y, "DPH (21%)")
    ...
    c.drawRightString(315, row_y - 5, format_czk(unit_price))
    
    c.setFillColor(colors.HexColor("#4c1d95"))
    c.roundRect(320, row_y - 10, 30, 12, 2, fill=1, stroke=0)
    c.setFillColor(colors.HexColor("#d8b4fe"))
    c.drawCentredString(335, row_y - 7, f"{format_units(discount_percent)}%")
    
    c.setFillColor(TEXT_COLOR)
    c.drawRightString(440, row_y - 5, format_czk(final_price))
    c.drawRightString(495, row_y - 5, format_czk(vat_amount))
    ...
    c.setFillColor(TEXT_COLOR)
    c.drawString(40, banner_y + 85, "Premiová konektivita")
    c.drawString(40, banner_y + 60, "Na 3 měsíce ")
    c.setFillColor(ACCENT_COLOR)
    c.drawString(40 + c.stringWidth("Na 3 měsíce ", "Roboto-Bold", 20), banner_y + 60, "Zdarma")
    ...
    for idx, b in enumerate(bullets):
        by = banner_y + 90 - (idx * 20)
        c.fillCircle(250, by + 3, 2)
        c.drawString(260, by, b)
        
    # App Store / Google Play style buttons (mocked with text boxes)
    btn_y = banner_y + 15
    ...
    # Let's try drawing an image on the right side
    decor_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "frontend", "public", "products", "Still_Strom-v1.png")
    if os.path.exists(decor_path):
        c.drawImage(decor_path, W - 180, banner_y - 20, width=160, preserveAspectRatio=True, mask='auto')

    # Footer numbers
    c.setFillColor(colors.HexColor("#475569"))
    c.setFont("Roboto", 10)
    c.drawCentredString(W / 2, 8, "4 z 6")
    c.showPage()
"""
...
print("Updated pdf_generator.py with top section spacing and new banner")
