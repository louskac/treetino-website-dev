...
const ZiggyConfig = {
    ...
    routes: {
        ...
        'products.treeV1': {
            uri: 'products/treetino-v1',
            methods: ['GET', 'HEAD'],
        },
        'products.treeV2': {
            uri: 'products/treetino-v2',
            methods: ['GET', 'HEAD'],
        },
        'products.turbine': {
            uri: 'products/turbine',
            methods: ['GET', 'HEAD'],
        },
...
const staticRoutes: Record<string, string | ((param?: any) => string)> = {
    ...
    'configurator.product': (id?: string) =>
        id ? `/configurator?product=${id}` : '/configurator',
    'sales.index': '/sales',
...
createInertiaApp({
    ...
    setup({ el, App, props, plugin }) {
        ...
        const activeMsgs =
            (i18nMessages as any)[activeLang] ||
            shared.value?.messages ||
            (i18nMessages as any)['cs'];
        ...
        if (isStaticMode) {
            ...
            router.on('invalid', (event) => {
                event.preventDefault();
                const detailUrl =
                    (event.detail as any)?.response?.config?.url ||
        ...
        const app = createApp({ render: () => h(App, props) })
            ...
            .use(
                CookieConsentPlugin,
                cookieConfig((key: string) =>
                    String((i18n.global as any).t(key)),
                ),
            )
