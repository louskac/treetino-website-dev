...
<template>
    <div class="fixed top-0 left-0 z-50 flex h-full w-full bg-black/70 p-6">
        <div
            class="mx-auto my-auto flex min-h-140 w-full flex-col overflow-hidden rounded-2xl bg-white shadow-xl lg:w-160 dark:bg-zinc-900"
        >
            <div
                class="border-b border-black/10 bg-t-accent/10 p-6 dark:border-white/10 dark:bg-white/5"
            >
            ...
            <div class="flex flex-1 flex-col p-6">
                <div v-if="step === 1" class="step-1">
                    <div class="">
                        ...
                        <input
                            v-model="email"
                            id="mail"
                            class="w-full rounded-xl border border-black/15 bg-white px-4 py-3 text-black transition-colors placeholder:text-black/40 focus:border-black focus:outline-none dark:border-white/15 dark:bg-zinc-800 dark:text-white dark:placeholder:text-white/40 dark:focus:border-white"
                            type="email"
                    ...
                    <div
                        class="pt-6 text-xs leading-relaxed text-black/60 dark:text-white/60"
                    >
                ...
                <div v-if="step === 2" class="step-2">
                    ...
                    <div v-if="canPayWithWallet" class="mb-6">
                        ...
                        >
                            <hr
                                class="w-full border-black/10 dark:border-white/10"
                            />
                            <span
                                class="absolute bg-white px-2 text-xs text-black/40 dark:bg-zinc-900 dark:text-white/40"
                                >{{
                                    $t('configurator.modal_checkout.or_card')
                                }}</span
                            >
                        </div>
                    </div>
                    ...
                    <label
                        class="mb-2 block text-sm font-medium text-black dark:text-white"
                        >{{
                            $t('configurator.modal_checkout.card_info')
                        }}</label
                    >
                    <div
                        id="card-element"
                        class="w-full rounded-xl border border-black/15 bg-white px-4 py-4 text-black dark:border-white/15 dark:bg-zinc-800 dark:text-white"
                    ></div>
                ...
                <div
                    class="mt-auto border-t border-black/10 pt-6 dark:border-white/10"
                >
                    ...
                    <div class="flex gap-2">
                        ...
                        >
                            {{
                                isProcessing
                                    ? $t(
                                          'configurator.modal_checkout.processing',
                                      )
                                    : step === 1
                                      ? $t(
